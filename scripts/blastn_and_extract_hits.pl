...
#
# Simplified version of add_homologs_to_aln.pl
#
# 1. Read in input alignment (protein)
# 2. Blast all sequences in alignment against NT dataset
#		ORFs from transcriptome)
# 1. parse BLASTN report
# 2. Select N best BLAST hits to file
# 3. Parse source fasta
# 4. Output hit fasta to original file (revcomp Plus/Minus hits)
# 
...
#	-Database (transcriptome): it will make headers unique
#
...
#		for f in *alns; do perl blastn_and_extract_hits.pl $f transcriptome.fa; done	
#	Multiple transcriptomes to multiple alignments:
#	for f in *alns; do for d in transcriptome.*fa; do perl blastn_and_extract_hits.pl $f $d; done	
#
##########################################################################################
...
my $usage = "perl blastn_and_extract_hits.pl alignment.fa database.fa // out to: alignment.hits.fa\n";
my $alignment = $ARGV[0] or die $usage;
my $database = $ARGV[1] or die $usage;
my $num_hits = "1"; # max number of final hits to retain per database
my $evalue = "1e-6"; # e-value threshold for BLASTP
my $threads = "1"; # number of threads for BLASTP
...
my	%database = read_fasta($database);
...
if ( !-e "$database") {
	...
	open(DB, ">", $database) or die "Can't create $database: $!";
...
if ( !-e "$database.phr" && !-e "$database.pin" && !-e "$database.psd" && 
     !-e "$database.pog" && !-e "$database.psi" && !-e "$database.psq" ) {

	print STDERR "\nCreating BLASTP database from: $database...\n";
	system("makeblastdb -in $database -dbtype nucl -parse_seqids");
}
else {
	
	print STDERR "\nBLASTP database already exists for: $database...\n";
}

# blastp

my $blast_out = $alignment_unaligned . "_vs_" . $database . ".blastn";

if ( !-e $blast_out ) {
    
    print STDERR "\nRunning BLASTN...\n";
    system("blastn -query $alignment_unaligned -db $database -evalue $evalue -num_descriptions 1 -num_alignments 1 -num_threads $threads -out $blast_out");
}
else {
    print STDERR "\nBLASTN results already exist for: $blast_out...\n";
}

# 3. parse blast report (save N best hits)
print STDERR "\nParsing BLASTN, retaining max. $num_hits hits...\n";
    ...
    while ( my $hit = $result->next_hit ) {
		...
		while( my $hsp = $hit->next_hsp ) {
			...
			if ( scalar keys %hits < $num_hits ) {
				...
				$hits{$hit_name}{'evalue'} = $evalue;
				$hits{$hit_name}{'strand'} = $strand; # +1 or -1
			}
			# replace hit if new hit has lower evalue
			else {
				foreach my $key ( keys %hits ) {
				
					if ( $evalue < $hits{'evalue'}{$key} ) {
					
						delete $hits{$key};
					$hits{$hit_name}{'evalue'} = $evalue;
					$hits{$hit_name}{'strand'} = $strand;
					}
				}	
			}
		}
...
print STDERR "\nExtracting best hits from: $database\nOutput appended to: $out_hits\n";
...
foreach my $h ( keys %hits ) {
	...
	if ( exists $database{$h} ) {
		...
		my $sequence = $database{$h};
		#print "$sequence\n";
		
		# reverse complement if Plus/Minus		
		if ( $hits{$h}{'strand'} == "-1" ) {
		
			$sequence = reverse $sequence;
			#print "$sequence\n";
			$sequence =~ tr/acgtunryACGTUNRY/tgcaanyrTGCAANYR/;
			#print "$sequence\n";
		}	
